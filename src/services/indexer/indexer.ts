...
export async function handleTransactions(
    ...
    while (true) {
        ...
        const syncRes = await retry(async () => await evaa.getSync(), { attempts: 5, attemptInterval: 1000 })
        if (!syncRes.ok) throw `Failed to sync evaa`

        console.log('Getting price data...')
        const pricesRes = await retry(async () => await evaa.getPrices(), { attempts: 15, attemptInterval: 1000 })
        ...
        for (const tx of transactions) {
            ...
            if (
                ...
                op === OP_CODE.MASTER_SUPPLY_SUCCESS ||
                ...
                if (op === OP_CODE.MASTER_LIQUIDATE_SATISFIED) {
                    ...
                    if (task !== undefined) {
                        ...
                        if (isEligibleTask) {
                            ...
                            await bot.sendMessage(
                                formatSwapAssignedMessage(loanAssetName, collateralAssetName, collateralRewardAmount, collateralAssetDecimals),
                                { parse_mode: 'HTML' }
                            )
                        } else {
                            await bot.sendMessage(
                                formatSwapCanceledMessage(loanAssetName, collateralAssetName, collateralRewardAmount, collateralAssetDecimals),
                                { parse_mode: 'HTML' }
                            )
                        }
                    }
                    ...
                    if (task !== undefined) {
                        ...
                        } else if (errorCode === ERROR_CODE.NOT_LIQUIDATABLE) {
                        ...
                        } else if (errorCode === ERROR_CODE.LIQUIDATION_PRICES_MISSING) {
                            // error message already logged
                        }
            ...
            setTimeout(async () => {
                ...
                const { codeVersion, ownerAddress: userAddress, principals } = openedUserContract.data
                ...
                const userRes = await retry(async () => await db.insertOrUpdateUser(actualUser), DATABASE_DEFAULT_RETRY_OPTIONS)
                if (!userRes) {
