...
import { JETTON_WALLETS, LIQUIDATION_BALANCE_LIMITS, POOL_CONFIG, USED_ASSETS_IDS_TO_LIQUIDATES } from '../../config'
...
export async function handleLiquidates(
    ...
    for (const task of tasks) {
        ...
        if (task.loan_asset === TON_MAINNET.assetId) {
            ...
            const asset = POOL_CONFIG.poolAssetsConfig.find((it) => it.assetId === task.loan_asset)
            if (!asset) {
